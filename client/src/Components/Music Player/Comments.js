...
const Comments = ({
    ...
    async function onSubmit() {
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({songID: id, comment: val})
        };
        const response = await fetch('http://localhost:4000/comment/song', requestOptions);
...
// POST request using fetch with async/await
        ...
        for (const element of arr) {
            if (!songNames.includes(element.comment.toString())) {
                setComs(songsArr => [...songsArr,
                    <div>
                ...
                songNames.push(element.comment.toString())
            }
        }
    }
